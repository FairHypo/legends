	    ...
	    <div class="row">
	    	...
	    	<div class="col-md-3 left-column">
	    		...
	    		<div class="chatter_sidebar">
					...
					@php
					$categories = DevDojo\Chatter\Models\Models::category()->all();

					$categoriesMenuNew = App\Services\ChatterHelper::categoriesMenu($categories->filter(function ($item) {
	        	...
	        	<div class="panel">
		        	<ul class="discussions">
		        		@foreach($discussions as $discussion)
				        	@can('forum-show-discussion', $discussion)
							<li>
				        		<a class="discussion_list" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ $discussion->category->slug }}/{{ $discussion->slug }}">
					        		<div class="chatter_avatar">
					        			@if(!empty($discussion->user->avatar))
					        				...
					        				@if( (substr($discussion->user->avatar, 0, 7) == 'http://') || (substr($discussion->user->avatar, 0, 8) == 'https://') )
					        					<img src="{{ $discussion->user->avatar  }}">
					        				@else
					        					<img src="{{ Config::get('chatter.user.relative_url_to_image_assets') . $discussion->user->avatar }}">
					        				@endif
					        			@else
					        		...
					        		<div class="chatter_middle">
					        			...
					        			<span class="chatter_middle_details">@lang('chatter::messages.discussion.posted_by') <span data-href="/user">{{ ucfirst($discussion->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</span> {{ $discussion->created_at }}</span>
	            ...
	            <div class="col-md-4">
					...
					<select id="chatter_category_id" class="form-control" name="chatter_category_id">
						...
						@foreach($categories as $category)
							@can('forum-show-category', $category)
